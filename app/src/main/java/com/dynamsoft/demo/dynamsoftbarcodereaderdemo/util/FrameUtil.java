...
public class FrameUtil {
	...
	public ArrayList<RectPoint[]> handlePoints(TextResult[] textResults, float previewScale, int srcBitmapHeight,int srcBitmapWidth) {
		...
		for (int i = 0; i < textResults.length; i++) {
			...
			point0.x = (textResults[i].localizationResult.resultPoints[0].x * previewScale)-(srcBitmapWidth * previewScale - viewWidth) / 2;
			point0.y = textResults[i].localizationResult.resultPoints[0].y * previewScale ;
			point1.x = (textResults[i].localizationResult.resultPoints[1].x * previewScale)-(srcBitmapWidth * previewScale - viewWidth) / 2;
			point1.y = textResults[i].localizationResult.resultPoints[1].y * previewScale ;
			point2.x = (textResults[i].localizationResult.resultPoints[2].x * previewScale)-(srcBitmapWidth * previewScale - viewWidth) / 2;
			point2.y = textResults[i].localizationResult.resultPoints[2].y * previewScale;
			point3.x = (textResults[i].localizationResult.resultPoints[3].x * previewScale)-(srcBitmapWidth * previewScale - viewWidth) / 2;
			point3.y =textResults[i].localizationResult.resultPoints[3].y * previewScale ;
			points[0] = point0;
			points[1] = point1;
			points[2] = point2;
			points[3] = point3;
